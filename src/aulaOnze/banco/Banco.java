...
public class Banco {
	...
	public static ContaCorrente acessaContaCorrente(ContaCorrente conta){
		...
		String resp;
		do{
			System.out.println("\n\nAcessando a conta "+conta.getIdentificador());
	...
	public static ContaPoupanca acessaContaPoupanca(ContaPoupanca conta){
		...
		do{
			...
			switch (resp){
				...
				case "s"->{
					System.out.println("Seu saldo é de "+String.format("%.2f",
									conta.getSaldo()));
				}
				case "r"->{
					System.out.print("Qual valor deseja sacar: ");
			...
			switch (resp){
				case "cp"->{
					...
					System.out.println("Conta cadastrada com identificador "+
									listaContaP[icp].getIdentificador());
					icp++;
				}
				case "cc"->{
					listaContaC[icc] = cadastraContaCorrente();
					System.out.println("Conta cadastrada com identificador "+
									listaContaC[icc].getIdentificador());
					icc++;
				}
				case "ap"->{
					...
					int posicao = -1;
					for(int i=0;i<icp;i++){
						if(listaContaP[i].getIdentificador().equals(identificador)
										&&listaContaP[i].getSenha().equals(senha)){
							posicao = i;
						}
					}
					if(posicao >= 0){
				...
				case "ac"->{
					...
					int posicao = -1;
					for(int i=0;i<icc;i++){
						if(listaContaC[i].getIdentificador().equals(identificador)
										&&listaContaC[i].getSenha().equals(senha)){
							posicao = i;
						}
					}
					if(posicao >= 0){
